...
| --- | --- |
| `/create` | 创建房间。 |
| `/room/:roomId/display` | 播放、二维码、下一首、seek 和 quota。 |
...
- Mobile 支持 `歌名 / 歌手`、会立即重搜的 `带原唱`、默认推荐、单个轻量 preview 和缓存内无限滚动。
- Search query、模式、结果、选中项、preview、scroll 和 tab 可恢复 24 小时。
- 点歌后保留搜索上下文；歌单支持置顶、删除、重唱和切歌。
- Display 自动尝试播放，提供 app-owned progress/seek、下一首和可靠的从头重播。
...
- Preview mute、autoplay、playsinline，使用小 viewport 让 YouTube 自适应码率；官方 API 无法强制锁定 360p。

Display：

- IFrame Player API + autoplay intent；API 未 ready 时等待。
- 短延迟 retry；browser block 时在 footer 显示独立播放按钮。
- 实际 PLAYING 才发送 `PLAYER_STARTED`；ENDED 发送 `PLAYER_ENDED`。
- Mobile restart 会重置 player flags/progress，再从 0 秒重新 load/play 并重新发送 `PLAYER_STARTED`；manual skip 和 natural end 共用推进规则。
- App-owned next、progress/seek；默认 iframe 禁止 pointer hover、关闭 native controls/fullscreen/keyboard，并移除 app 自己覆盖在视频上的状态、点击和结束遮罩。
- 默认 `画质：自动（推荐）`，由 YouTube 根据用户网络和设备自适应；选择 `画质：手动设置` 后显示 YouTube 原生 controls，由用户在原生设置菜单中选当前可用清晰度，选择会在连续歌曲间保留。
...
9. 测置顶、删除、重唱、切歌。
10. Display 测 seek、restart 和 next。
...
- 默认显示 `画质：自动（推荐）`；手动模式只打开 YouTube 原生画质菜单，不伪装成可强制固定的 app quality API。
...
8. Autoplay/restart/seek 必须在目标浏览器复现；YouTube 画质验证默认 adaptive 与手动 native controls 两种模式，不验证已废弃的强制 quality API。
