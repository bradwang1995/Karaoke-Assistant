...
- App-owned next、progress/seek；iframe 禁止 pointer hover，减少 title/share/end-screen chrome。
- Google 已明确 `setPlaybackQuality`、`getPlaybackQuality` 和 `getAvailableQualityLevels` 不再支持，`suggestedQuality/vq` 也会被忽略；因此本项目移除了会误导用户的假清晰度 selector，画质由 YouTube 自适应决定。

官方说明：<https://developers.google.com/youtube/iframe_api_reference#october-24,-2019>
...
8. Autoplay/restart/seek 必须在目标浏览器复现；YouTube 画质只验证自适应播放，不验证已废弃的强制 quality API。
